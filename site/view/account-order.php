            ...
            <a href="index.php?page=order_detail&order_code=<?php echo $order['order_code'] ?>">
                <div class="order__item">
                    ...
                    <div class="order__container">
                        <?php
                        $sql_order_detail_list = "SELECT od.order_detail_id,od.variant_id, p.product_id, p.product_name, od.product_quantity, od.product_price, od.product_sale, p.product_image FROM order_detail od JOIN product p ON od.product_id = p.product_id WHERE od.order_code = '" . $order['order_code'] . "' ORDER BY od.order_detail_id DESC";
                        ...
                        ?>
                            <div class="cart__item d-flex align-center">
                                ...
                                <div class="d-flex flex-1 align-center" style="gap: 16px;">
                                    <div>
                                        ...
                                        <span class="cart__quantity h6 d-block">Số lượng: <?php echo $order_detail['product_quantity'] ?></span>
                                    </div>
                                ...
                                <?php
                                    if ($order_detail['product_sale'] > 0) {
                                    ?>
                                        <span class="cart__color"></span><del class="product__price--old cart__color "><?php echo number_format($order_detail['product_price']) . ' ₫' ?></del>
                                    <?php
                                    }
                                    ?>
                                    <span class="product__price--new cart__color"><?php echo (number_format($order_detail['product_price'] - ($order_detail['product_price'] / 100 * $order_detail['product_sale']))) . ' ₫' ?></span>
