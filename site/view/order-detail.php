...
$sql_order_detail_list = "SELECT od.order_detail_id,od.variant_id, p.product_id, p.product_name, od.product_quantity, od.product_price, od.product_sale, p.product_image FROM order_detail od JOIN product p ON od.product_id = p.product_id WHERE od.order_code = '" . $order_code . "' ORDER BY od.order_detail_id DESC";
        ...
        <div class="row">
                ...
                <div class="checkout__cart" style="padding-block: 0;">
                    ...
                    <div class="checkout__items">
                        ...
                        while ($cart_item = mysqli_fetch_array($query_order_detail_list)) {
                            $sale += ($cart_item['product_price'] / 100 * $cart_item['product_sale']) * $cart_item['product_quantity'];
                        ...
                        ?>
                            <div class="checkout__item d-flex align-center">
                                ...
                                <div class="h6 checkout__price"><?php echo (number_format(($cart_item['product_price'] - ($cart_item['product_price'] / 100 * $cart_item['product_sale']))* $cart_item['product_quantity'])) . ' ₫' ?></div>
                            </div>
                        <?php
                        }
                        ?>
                    ...
                    <table class="w-100 mg-t-20">
                        ...
                        <tr>
                            <td class="h6">Tiết kiệm</td>
                            <td class="h6 text-right"><?php echo number_format((float) $sale) . '₫' ?></td>
                        </tr>
                        <tr>
                    ...
                    <div class="order__detail--status h4">
                        Trạng thái đơn hàng: <?php echo format_order_status($order['order_status']) ?>
                    </div>
                </div>
            </div>
            <div class="w-100 d-flex align-center space-between">
                <div class="order__detail--action">
                    ...
                    ?>
                        <a href="site/handle/order.php?order_cancel=1&order_code=<?php echo $order_code ?>" class="btn btn__solid" onClick="return confirm('Bạn có muốn hủy đơn hàng này không?')">Hủy đơn hàng</a>
                    <?php
                    } elseif ($order['order_status'] == 3) {
                    ?>
                        <a href="site/handle/order.php?order_confirm=1&order_code=<?php echo $order_code ?>" class="btn btn__solid" onClick="return confirm('Xác nhận đã nhận được hàng?')">Xác nhận đã nhận hàng</a>
                    <?php
                    } else {
                    ?>
                    ...
                    <a href="index.php?page=my_account&tab=account_order" class="btn__back anchor">Trở về danh sách đơn hàng</a>
                </div>
