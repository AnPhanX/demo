<?php
		...
		foreach($_SESSION['cart'] as $cart_item){
			if($cart_item['product_id']==$product_id && $cart_item['variant_id']==$product_type){
				...
				if($cart_item['product_quantity']<$quantity){
					$product[]= array('product_id'=>$cart_item['product_id'], 'variant_id'=>$cart_item['variant_id'], 'product_name'=>$cart_item['product_name'],'product_quantity'=>$total_count,'product_price'=>$cart_item['product_price'],'product_sale'=>$cart_item['product_sale'],'product_image'=>$cart_item['product_image']);
				}else{
	...
	if(isset($_POST['addtocart'])){
		...
		// $variant_id=$_POST['product_scent'];
		$variant_id=$_POST['product_scent'];
		...
		if($row){
			$new_product=array(array('product_id'=>$product_id, 'variant_id'=>$variant_id, 'product_name'=>$row['product_name'],'product_quantity'=>$product_quantity,'product_price'=>$row['product_price'], 'product_sale'=>$row['product_sale'],'product_image'=>$row['product_image']));
			...
			if(isset($_SESSION['cart'])){
				...
				foreach($_SESSION['cart'] as $cart_item){
					...
					if($cart_item['product_id']==$product_id && $cart_item['variant_id']==$variant_id){
						...
						$product[]= array('product_id'=>$cart_item['product_id'], 'variant_id'=>$cart_item['variant_id'], 'product_name'=>$cart_item['product_name'],'product_quantity'=>$cart_item['product_quantity'],'product_price'=>$cart_item['product_price'],'product_sale'=>$row['product_sale'],'product_image'=>$cart_item['product_image']);
					}
				}
				if($found == false){
