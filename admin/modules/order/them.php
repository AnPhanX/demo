                ...
                <div class="card-content">
                    <form action="modules/order/xuly.php" method="POST">
                        ...
                        <div class="input-item form-group">
                            ...
                            <select name="product_id" id="productid" class="form-control select_product" onchange="updateVariantList()" required>
                        ...
                        <div class="input-item form-group">
                            <label for="variantid" class="d-block">Loại sản phẩm</label>
                            <select name="variant_id" id="variantid" class="form-control select_variant" required>
                                <!-- Các option sẽ được cập nhật thông qua JavaScript -->
                            </select>
                        </div>
                        <div class="input-item form-group">
...
<script>
    function updateVariantList() {
        // Lấy giá trị từ select box phía trên
        var productId = document.getElementById("productid").value;

        // Gửi yêu cầu AJAX để lấy danh sách các biến thể dựa trên sản phẩm được chọn
        var xhr = new XMLHttpRequest();
        xhr.onreadystatechange = function() {
            if (xhr.readyState == 4 && xhr.status == 200) {
                // Cập nhật select box phía dưới với danh sách biến thể mới
                var variantSelect = document.getElementById("variantid");
                // Đặt option mặc định rỗng và cập nhật các option từ response
                variantSelect.innerHTML = '<option value="" selected>Chọn size</option>' + xhr.responseText;
            }
        };
        xhr.open("GET", "./modules/order/get_variant_list.php?product_id=" + productId, true);
        xhr.send();
    }
</script>