<?php
    ...
    require('../../../system/library/tfpdf/tfpdf.php');
	...
	$pdf->Ln(10);
	$pdf->Cell(0,0,'HÓA ĐƠN BÁN HÀNG DICO STORE',0,0,'C');
	$pdf->Ln(20);
	...
	while($row = mysqli_fetch_array($query_order_detail_list)){
		$sql_variant = "SELECT * FROM product_variant WHERE product_id = '".$row['product_id']."' LIMIT 1";
	...
	$pdf->Cell($width_cell[3],10,' (' . $variant['variant_name'] . ')',1,0,'C',$fill);
	$pdf->Cell($width_cell[4],10,$row['product_quantity'],1,0,'C',$fill);
	$pdf->Cell($width_cell[5],10,number_format($row['product_price'] - ($row['product_price'] / 100 * $row['product_sale'])).'đ',1,0,'C',$fill);
	$pdf->Cell($width_cell[6],10,number_format(($row['product_price'] - ($row['product_price'] / 100 * $row['product_sale']))*$row['product_quantity'])."đ",1,1,'C',$fill);
	$fill = !$fill;
    ...
    $pdf->Write(10,'DiCo Store cảm ơn quý khách hàng!');
	$pdf->Ln(10);
