...
if (isset($_POST['addtoorder'])) {
    ...
    if ($row && $row['product_quantity'] >= $product_quantity) {
        $new_product = array(array('product_id' => $product_id, 'product_name' => $row['product_name'], 'variant_id' => $variant_id, 'product_quantity' => $product_quantity, 'product_price' => $row['product_price'], 'product_sale' => $row['product_sale'], 'product_image' => $row['product_image']));
        if (isset($_SESSION['order'])) {
            $found = false;
            foreach ($_SESSION['order'] as $order_item) {
                //neu du lieu trung
                if ($order_item['product_id'] == $product_id && $order_item['variant_id'] == $variant_id) {
                    $product[] = array('product_id' => $order_item['product_id'], 'product_name' => $order_item['product_name'], 'variant_id' => $order_item['variant_id'], 'product_quantity' => $order_item['product_quantity'] + $product_quantity, 'product_price' => $order_item['product_price'], 'product_sale' => $order_item['product_sale'], 'product_image' => $order_item['product_image']);
                    $found = true;
                } else {
                    //neu du lieu khong trung
                    $product[] = array('product_id' => $order_item['product_id'], 'product_name' => $order_item['product_name'], 'variant_id' => $variant_id, 'product_quantity' => $order_item['product_quantity'], 'product_price' => $order_item['product_price'], 'product_sale' => $order_item['product_sale'], 'product_image' => $order_item['product_image']);
                }
            }
            if ($found == false) {
